...
<body>
	...
	header("Content-Disposition: attachment; filename=Riwayat Transaksi Bulanan.xls");
	?>
 
	<center>
		<h1>Laporan Bulanan <br/> Rumah Singgah</h1>
	</center>
	...
	<table border="2">
		...
		$date = date("y-m-d");
                        	 $sql = mysqli_query($conn,"SELECT * FROM selesai WHERE YEAR(tanggal) = YEAR(CURDATE()) AND MONTH(tanggal) = MONTH(CURDATE())");
